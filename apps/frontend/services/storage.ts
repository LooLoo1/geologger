...
export const saveLocationOffline = async (location: LocationLog): Promise<void> => {
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      const request = store.add({
        ...location,
        synced: false,
        id: `${location.userId}_${location.timestamp}`,
      });
...
export const getUnsyncedLocations = async (): Promise<LocationLog[]> => {
  try {
    const database = await initDB();
...
export const markLocationsAsSynced = async (locationIds: string[]): Promise<void> => {
  try {
    const database = await initDB();
    const transaction = database.transaction([STORE_NAME], 'readwrite');
    const store = transaction.objectStore(STORE_NAME);
